...
def _build_render_user_prompt(inp: TreatmentPlanInput, plan_json: dict) -> str:
    ...
    if protocol_block:
        protocol_guidance = (
            ...
            "- Use 'completed_sessions' vs. 'typical_sessions' in the JSON to gauge the patient's current "
            "phase within each active protocol. Let this shape the plan's goals and next steps naturally: "
            "early phase → focus on assessment, goal-setting, and foundational skills; "
            "middle phase → active skill application and experiments; "
            "late phase → consolidation, relapse prevention, and gradual termination planning.\n"
            "- Do not state numeric session counts or percentages in the Hebrew output; reflect the phase "
            "naturally through the language and choice of goals.\n"
