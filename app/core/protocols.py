...
SYSTEM_PROTOCOLS: List[Protocol] = [
    ...
    # ── Speech-Language Pathology (קלינאות תקשורת) ──────────────────────────

    Protocol(
        id="slp_articulation",
        name="פרוטוקול הגייה בסיסי",
        approach_id="slp_phonological_articulation",
        target_problem="שיבושי הגייה פונקציונליים בצליל אחד או יותר",
        description=(
            "פרוטוקול הגייה בסיסי לטיפול בשיבושי צליל: תרגול שיטתי מהצליל המבודד "
            "דרך הברות, מילים ומשפטים — עם מודלינג, שיקוף ומשחקי הגייה."
        ),
        typical_sessions=12,
        core_techniques=[
            "תרגול צליל מבודד → הברות → מילים → משפטים",
            "מודלינג ושיקוף מול מראה",
            "משחקי הגייה עם קלפים ותמונות",
            "חיזוק חיובי ומשוב מיידי",
        ],
    ),

    Protocol(
        id="slp_language_delays",
        name="פרוטוקול עיכוב שפתי מוקדם",
        approach_id="slp_communicative_social",
        target_problem="עיכוב בהבנה ובהבעה אצל פעוטות וילדים צעירים",
        description=(
            "פרוטוקול לטיפול בעיכוב שפתי מוקדם: הרחבת אמירות הילד, "
            "משחק מונחה בסיטואציות יומיומיות והדרכת הורים."
        ),
        typical_sessions=14,
        core_techniques=[
            "הרחבת אמירות הילד (expansions, recasts)",
            "משחק מונחה סביב סיטואציות יומיומיות",
            "הדרכת הורים ליצירת הזדמנויות תקשורת בבית",
            "מודלינג שפתי בהקשר טבעי",
        ],
    ),

    Protocol(
        id="slp_phonological_processes",
        name="פרוטוקול תהליכים פונולוגיים",
        approach_id="slp_phonological_articulation",
        target_problem="תהליכים פונולוגיים מתמשכים (השמטות, החלפות, שיבוש קבוצות עיצורים)",
        description=(
            "פרוטוקול לתהליכים פונולוגיים: עבודה עם זוגות מילים (minimal pairs), "
            "תרגילי הבחנה שמיעתית ומודעות פונולוגית, ותרגול מובנה בהקשרים שונים."
        ),
        typical_sessions=15,
        core_techniques=[
            "עבודה עם זוגות מילים (minimal pairs)",
            "תרגילי הבחנה שמיעתית ומודעות פונולוגית",
            "תרגול מובנה של צלילים בהקשרים שונים",
            "פסיכוחינוך להורים על תהליכים פונולוגיים",
        ],
    ),
]
